...
export function DoDocuments({ doId }: { doId: string }) {
  ...
  useEffect(() => {
    apiFetch<DocumentFile[]>(`/api/v1/delivery-order/${doId}/documents`)
  ...
  const handleUpload = async (file: File) => {
    ...
    try {
      ...
      const r = await apiFetchFormData(`/api/v1/delivery-order/${doId}/documents`, formData)
  ...
  const handleDelete = async (docId: string) => {
    try {
      await apiFetch(`/api/v1/delivery-order/${doId}/documents?docId=${docId}`, { method: "DELETE" })
