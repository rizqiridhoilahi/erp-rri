...
export default async function CustomerPoDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: po, error } = await supabase.from('customer_po').select('*, customer!customer_id(nama, kode)').eq('id', id).single()
  if (error || !po) return <div className="text-center py-20 text-muted-foreground">Customer PO tidak ditemukan</div>
  const { data: items } = await supabase.from('customer_po_item').select('*, barang!barang_id(nama, kode, satuan)').eq('customer_po_id', id)

  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Button variant="ghost" size="icon" asChild><Link href="/dashboard/customer-po"><ArrowLeft className="h-5 w-5" /></Link></Button>
