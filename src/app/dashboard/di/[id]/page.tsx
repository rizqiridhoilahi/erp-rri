...
export default async function DiDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params
  const { data: di, error } = await supabase.from('di').select('*, customer!customer_id(nama, kode)').eq('id', id).single()
  if (error || !di) return <div className="text-center py-20 text-muted-foreground">DI tidak ditemukan</div>
  const { data: items } = await supabase.from('di_item').select('*, barang!barang_id(nama, kode, satuan)').eq('di_id', id)

  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Button variant="ghost" size="icon" asChild><Link href="/dashboard/di"><ArrowLeft className="h-5 w-5" /></Link></Button>
