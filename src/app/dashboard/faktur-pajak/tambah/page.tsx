...
const itemSchema = z.object({ invoice_item_id: z.string().min(1), harga_satuan: z.coerce.number().positive(), dpp: z.coerce.number().positive(), ppn: z.coerce.number().positive(), pph: z.coerce.number().optional() })
...
export default function TambahFakturPajakPage() {
  ...
  const form = useForm<FV>({ resolver: zodResolver(schema), defaultValues: { tanggal: today, items: [{ invoice_item_id: '', harga_satuan: 0, dpp: 0, ppn: 0 }] } })
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          ...
          <Card><CardHeader className="flex flex-row items-center justify-between"><CardTitle className="text-base">Item Invoice</CardTitle>
            <Button type="button" variant="outline" size="sm" onClick={() => append({ invoice_item_id: '', harga_satuan: 0, dpp: 0, ppn: 0 })}><Plus className="h-4 w-4 mr-1" />Tambah</Button></CardHeader>
            <CardContent className="space-y-4">{fields.map((f, i) => (
              <div key={f.id} className="p-4 border rounded-lg space-y-3"><div className="flex items-center justify-between"><span className="text-sm text-muted-foreground">Item #{i+1}</span>{fields.length > 1 && <Button type="button" variant="ghost" size="sm" onClick={() => remove(i)}><Trash2 className="h-4 w-4 text-destructive" /></Button>}</div>
                <div className="grid grid-cols-4 gap-3">
                  ...
                  <div className="space-y-2"><label className="text-xs font-medium">Harga Satuan *</label><Input type="number" step="0.01" {...form.register(`items.${i}.harga_satuan`)} /></div>
