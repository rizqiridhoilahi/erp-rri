    ...
    <>
      ...
      <section ref={statsRef} className="relative z-20 -mt-16 max-w-[1280px] mx-auto px-[40px]">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 bg-white/95 backdrop-blur-xl border border-[#c5c4db]/30 rounded-xl p-8 shadow-2xl shadow-[#0B1528]/5">
          {stats.map((stat, i) => (
            <div key={i} className="flex flex-col items-center text-center space-y-3 border-r border-[#e0e3e5]/20 last:border-0">
              <svg className={`w-6 h-6 text-[#0000ff] ${statsInView ? 'stat-visible' : ''}`} style={{ animationDelay: `${i * 100}ms` }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
      ...
      <section ref={layananRef} className="py-24 bg-[#f7f9fb]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {services.map((service, i) => (
              <Link
                key={i}
                href={service.href}
                className={`glass-card rounded-xl p-8 hover:translate-y-[-6px] hover:shadow-xl transition-all duration-300 cursor-pointer group block ${layananVisible ? 'reveal-visible' : 'reveal'}`}
                style={{ animationDelay: `${i * 150}ms` }}
              >
                <svg className="w-10 h-10 text-[#0000ff] mb-4 group-hover:scale-110 transition-transform duration-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
      ...
      <section ref={keunggulanRef} className="py-24 bg-white">
        <div className="max-w-[1280px] mx-auto px-[40px]">
          <div className="text-center mb-16">
            <h2 className="text-[32px] font-bold text-[#191c1e] mb-4 font-[family-name:var(--font-heading)]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {keunggulan.map((item, i) => (
              <div
                key={i}
                className={`rounded-xl p-6 border border-[#c5c4db]/20 hover:border-[#343DFF]/30 hover:shadow-lg hover:shadow-[#343DFF]/5 transition-all duration-300 ${keunggulanVisible ? 'reveal-visible' : 'reveal'}`}
                style={{ animationDelay: `${i * 120}ms` }}
              >
                <div className="w-12 h-12 rounded-lg bg-[#343DFF]/10 flex items-center justify-center mb-4 group-hover:bg-[#343DFF]/20 transition-colors duration-300">
                  <svg className="w-6 h-6 text-[#0000ff]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                ...
                <h3 className="text-[18px] font-semibold text-[#191c1e] mb-2 font-[family-name:var(--font-heading)]">
                  {item.title}
                </h3>
                <p className="text-[#454558] text-[14px] leading-relaxed font-[family-name:var(--font-body)]">
                  {item.desc}
      ...
      <section ref={klienRef} className="py-24 bg-[#f7f9fb]">
        <div className="max-w-[1280px] mx-auto px-[40px]">
          <div className="text-center mb-16">
            <h2 className="text-[32px] font-bold text-[#191c1e] mb-4 font-[family-name:var(--font-heading)]">
              {dict.klien.title}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {[...Array(4)].map((_, i) => (
              <div
                key={i}
                className={`h-24 rounded-xl bg-white border border-[#c5c4db]/20 flex items-center justify-center hover:border-[#343DFF]/30 hover:shadow-md transition-all duration-300 ${klienVisible ? 'logo-placeholder' : 'opacity-0'}`}
                style={{ animationDelay: `${i * 150}ms` }}
              >
                <span className="text-[#94A3B8] text-[14px] font-medium font-[family-name:var(--font-body)]">
                  Logo Client {i + 1}
                </span>
              </div>
            ))}
        ...
        <div className="max-w-[1280px] mx-auto px-[40px]">
          ...
          <div className="max-w-3xl mx-auto">
            {dict.testimonial.items.map((item, i) => (
              ...
              >
                <div className="absolute -top-4 -left-2 text-[#343DFF]/20">
                  <svg className="w-12 h-12" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M14.017 21v-7.391c0-5.704 3.731-9.57 8.983-10.609l.995 2.151c-2.432.917-3.995 3.638-3.995 5.849h4v10H14.017zM0 21v-7.391c0-5.704 3.731-9.57 8.983-10.609l.995 2.151C7.546 6.068 5.983 8.789 5.983 11H10v10H0z" />
                  </svg>
                </div>
                <div className="bg-[#f7f9fb] rounded-xl p-8 border border-[#c5c4db]/20">
                  <p className="text-[#454558] text-[16px] leading-relaxed italic mb-6 font-[family-name:var(--font-body)]">
                    &ldquo;{item.quote}&rdquo;
                  </p>
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-full bg-gradient-to-br from-[#343DFF] to-[#0000FF] flex items-center justify-center text-white font-bold text-[16px] font-[family-name:var(--font-heading)]">
                      {item.name.charAt(0)}
                    </div>
                    <div>
                      ...
                      <p className="text-[#94A3B8] text-[13px] font-[family-name:var(--font-body)]">
                        {item.role}
                      </p>
                    </div>
