    ...
    <div>
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-6">
        <div>
          <nav className="flex items-center gap-2 text-xs text-[#555e75] mb-2">
            ...
            <span className="text-[#0001bb] font-medium">{dict.portal.sphHistory}</span>
          </nav>
      ...
      {data?.stats && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          <div className="bg-white/80 backdrop-blur-[12px] border border-[#c5c4db]/30 rounded-xl p-5 shadow-sm border-l-4 border-l-[#0001bb]">
          ...
          <div className="bg-white/80 backdrop-blur-[12px] border border-[#c5c4db]/30 rounded-xl p-5 shadow-sm border-l-4 border-l-green-500">
          ...
          <div className="md:col-span-2 bg-[#0000ff]/5 border border-[#c5c4db]/30 rounded-xl p-5 shadow-sm relative overflow-hidden">
      ...
      <div className="bg-white rounded-xl border border-[#c5c4db]/30 overflow-hidden shadow-sm">
        <div className="p-4 border-b border-[#c5c4db]/30">
          <div className="relative max-w-md">
            <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-[#757589]">search</span>
            <input
              type="text"
              value={search}
              onChange={e => { setSearch(e.target.value); setPage(1) }}
              placeholder={dict.portal.search}
              className="w-full pl-10 pr-4 py-2 bg-[#f2f4f6] border border-[#c5c4db]/50 rounded-full text-sm focus:ring-2 focus:ring-[#0001bb]/20 focus:border-[#0001bb] outline-none transition-all font-[family-name:var(--font-body)]"
            />
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center p-12 text-center">
            <span className="material-symbols-outlined text-5xl text-[#757589] mb-3">request_quote</span>
              ...
              <tbody className="divide-y divide-[#c5c4db]/20">
                {data.rfqs.map((rfq) => (
                  <tr key={rfq.id} className="hover:bg-[#f2f4f6]/30 transition-colors">
                    <td className="px-6 py-4 text-sm font-semibold text-[#0001bb] font-[family-name:var(--font-body)]">{rfq.nomor}</td>
                      ...
                      {rfq.pdfUrl ? (
                        <button
                          onClick={() => {
                            ...
                            fetch(rfq.pdfUrl!, { headers: { Authorization: `Bearer ${token}` } })
                              .then(res => res.blob())
                              .then(blob => { win.location.href = URL.createObjectURL(blob) })
                              .catch(() => win.close())
                          }}
                          className="bg-[#0000ff] hover:bg-[#0000cc] text-white px-4 py-2 rounded-lg font-semibold text-xs shadow-lg shadow-[#0000ff]/20 flex items-center gap-1.5 ml-auto transition-all active:scale-95"
                        >
      ...
      <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-white/80 backdrop-blur-[12px] border border-[#c5c4db]/30 rounded-xl p-6 relative overflow-hidden">
          <div className="relative z-10">
            ...
            <a href="mailto:info@pt-rri.com" className="text-[#0001bb] font-bold text-sm flex items-center gap-2 hover:gap-3 transition-all font-[family-name:var(--font-body)]">
              Contact Support
              <span className="material-symbols-outlined text-[16px]">arrow_right_alt</span>
            </a>
          </div>
          <div className="absolute right-0 top-0 w-32 h-32 bg-[#0001bb]/5 rounded-full -translate-y-1/2 translate-x-1/2"></div>
        </div>
        <div className="bg-white/80 backdrop-blur-[12px] border-2 border-dashed border-[#c5c4db]/50 rounded-xl p-6">
          <div className="flex items-start gap-4">
            <div className="p-3 bg-[#e0e3e5] rounded-lg">
              <span className="material-symbols-outlined text-[#0001bb]">info</span>
            </div>
            <div>
