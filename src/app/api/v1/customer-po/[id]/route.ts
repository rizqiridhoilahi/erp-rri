...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (body.status === 'confirmed' && old?.status !== 'confirmed') {
    ...
    // Send WhatsApp + Email notification to PIC Customer
    ...
    if (poWithCustomer?.customer) {
      const { data: pics } = await supabaseAdmin.from('customer_pic').select('no_hp, nama, email').eq('customer_id', poWithCustomer.customer_id).eq('is_active', true).limit(1)
      const pic = pics?.[0]
      if (pic?.no_hp) {
