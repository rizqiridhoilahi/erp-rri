-- Add revisi suffix (-R1, -R2, etc.) to Quotation virtual PDF filename in all_documents view
...
JOIN customer c ON c.id = p.customer_id
WHERE (d.document_type IS NULL OR d.document_type != 'grn_customer')
...
JOIN invoice p ON p.id = d.invoice_id
JOIN customer c ON c.id = p.customer_id
WHERE d.document_type = 'grn_customer'
...
-- GRN Supplier documents (customer resolved via DI if set)
...
SELECT
  'pdf-quotation-' || p.id,
  'Quotation - ' || p.nomor || CASE WHEN COALESCE(p.revisi, 0) > 0 THEN ' -R' || p.revisi ELSE '' END || '.pdf',
...
-- Per-term invoice PDFs (multi-termin)
