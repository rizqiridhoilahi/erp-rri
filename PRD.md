...
- Setiap tahun berganti, counter di-reset ke `0001`
...
- Contoh: DI `RRI-DI-26-06-0001` → Quotation anaknya `RRI-SPH-26-06-0001`
...
- Quotation (dari RFQC) → `formatChildNumber(parentNumber, 'SPH')` → `RRI-SPH-26-05-0001`
...
- `generateGlobalDocumentNumber(kodeDokumen)` — increment global counter, return `RRI-{KODE}-YY-MM-NNNN`
- `formatChildNumber(parentNumber, kodeDokumen)` — ekstrak nomor urut dari parent, ganti prefix → `RRI-{KODE}-YY-MM-NNNN`
